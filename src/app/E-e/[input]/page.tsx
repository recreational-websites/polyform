...
import { decode } from "@/_internal/lib/E/e/decode";
import { encode } from "@/_internal/lib/E/e/encode";
import { moreInfo } from "@/_internal/lib/E/e/moreInfo";
import { notFound, redirect } from "next/navigation";
import { env } from "../../../_internal/lib/env";
import { Page as InternalPage } from "../../../_internal/pages/E-e/Page";
...
export default async function Page({ params }: Params) {
  ...
  if (name !== input) throw redirect(`/E-e/${name}`);
...
export function generateMetadata({ params }: Params): Metadata {
  ...
  const title = `E edge ${input}`;
  const description = `E edge ${input} with symmetry group ${symmetryGroup}`;
  ...
  return {
    ...
    openGraph: {
      title,
      images: `${env("METADATA_BASE")}/polyform/api/E-e/og/${input}`,
      description,
    },
  };
}
